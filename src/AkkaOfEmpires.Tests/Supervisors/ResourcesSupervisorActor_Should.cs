...
namespace AkkaOfEmpires.Tests.Supervisors
{
    public class ResourcesSupervisorActor_Should : TestKit
    {
        ...
        public void Increase_FoodAmount_When_FoodGathered_Message_Received()
        {
            ...
            var message = new ResourceGathered { ResourceType = Resource.Food, Quantity = 10 };
            actor.Tell(message);
        ...
        {
            var actor = ActorOfAsTestActorRef<ResourcesSupervisorActor>();
            var message = new ResourceGathered { ResourceType = Resource.Wood, Quantity = 12 };
            actor.Tell(message);
        ...
        {
            var actor = ActorOfAsTestActorRef<ResourcesSupervisorActor>();
            var message = new ResourceGathered { ResourceType = Resource.Gold, Quantity = 8 };
            actor.Tell(message);
        ...
        {
            var actor = ActorOfAsTestActorRef<ResourcesSupervisorActor>();
            var message = new ResourceGathered { ResourceType = Resource.Stone, Quantity = 5 };
            actor.Tell(message);
